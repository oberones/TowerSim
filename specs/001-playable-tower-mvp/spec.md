...
- Q: When both stairs and elevators provide a valid route, which should travelers prefer? → A: Walk on the same floor; prefer an available stairs-only route for trips spanning one or two floors and an elevator route for longer trips. Use another valid route if the preferred type is unavailable. A long elevator queue alone does not trigger switching to stairs.
...
4. **Given** valid stairs-only and elevator routes from the lobby to destinations one, two, and three floors above it, **When** travelers choose routes, **Then** they use stairs for the one- and two-floor trips and an elevator for the three-floor trip; connected same-floor journeys use walking.
