...
Apply mode preference first: same-floor direct walking when connected; the nearest usable stair/elevator entrance by walking approach for one/two floors; a route containing elevator travel for longer trips if viable; otherwise another valid route. Inside that class, use walk/stair duration, estimated wait, ride duration and transfer penalty. Estimate wait from a nominal cycle and directional queued plus approaching assignments, not a full future car simulation. Sequential routing updates assignments to avoid a same-tick herd.
