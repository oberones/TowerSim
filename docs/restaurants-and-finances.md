...
Choose **Restaurant**, click constructed floor space, and confirm **Place restaurant** on the room. Its footprint is automatic. The first click only previews placement; Cancel or Escape leaves cash and geometry unchanged. Click a built room in Inspect mode to reveal its tab. **Tower details** also opens the reports; **Close details** hides them again.

Before confirming, drag the room preview or its **Move** handle to adjust placement. Arrow keys on **Move** nudge by one cell or floor. The room keeps its size, and its price and validity are checked again at the new location.
