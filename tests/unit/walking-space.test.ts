...
test('touching spans connect, gaps break paths, and upper construction alone creates no lobby route',()=>{
  ...
  expect(hasWalkingPath(s.tower!,1,2,38)).toBe(false);expect(accessAt(s,1,2)).toMatchObject({accessible:false,reason:'No stairs or elevator connection to the lobby'});
