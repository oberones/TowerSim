...
test.each(['outside','walking','inside'] as const)('demolition during %s preserves physical exits and clears live facility references',phase=>{const {s,id}=leasedWorker();const o=Object.keys(s.offices)[0]!,q=s.occupants[id]!.schedule!;if(phase!=='outside')until(s,q.arrivalTick+(phase==='inside'?OFFICE_WALK_TICKS:10));expect(command(s,{kind:'demolishEntity',payload:{entityId:o}}).ok).toBe(true);expect(s.offices[o]).toBeUndefined();expect(s.scheduledEvents.filter(e=>e.targetId===id).every(e=>e.kind==='walkComplete')).toBe(true);if(phase==='walking')expect(s.occupants[id]!.replanAfterCurrentLeg).toBe(true);encodeState(s);until(s,s.clock.tick+2*OFFICE_WALK_TICKS);expect(s.occupants[id]).toBeUndefined();expect(s.officeMarket.pendingRelease).toBe(1);expect(Object.values(s.trips).every(t=>t.endTick!==null)).toBe(true);});
...
test('a demolished indoor worker stranded by a gap survives until repair and a real exit',()=>{
 ...
 floor(s,0,8,40);const o=place(s);advance(s,1);const id=Object.keys(s.occupants)[0]!;until(s,s.occupants[id]!.schedule!.arrivalTick+OFFICE_WALK_TICKS);
 ...
 floor(s,0,8,16);expect(s.occupants[id]!.state).toBe('walking');until(s,s.clock.tick+OFFICE_WALK_TICKS);expect(s.occupants[id]).toBeUndefined();expect(Object.values(s.trips).some(t=>t.totals.stranded===100)).toBe(true);encodeState(s);
});
