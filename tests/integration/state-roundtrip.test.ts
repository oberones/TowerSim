...
test.each([21600,36000,36900,38100,86399,86400])('restaurant visits, payments and wakeups continue exactly from %i',at=>{const s=oneCustomer();until(s,at);const copy=decodeState(encodeState(s));rebuildDerived(copy);until(s,2*86400);until(copy,2*86400);expect(encodeState(copy)).toBe(encodeState(s));});
test('old transport-only snapshots are rejected instead of inventing finance or customer state',()=>{const s=oneCustomer(),old=JSON.parse(encodeState(s));old.rulesetId='tower-transport-v1';old.contentVersion='mvp-transport-v1';expect(validateState(old).ok).toBe(false);});
