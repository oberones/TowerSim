...
For an office or restaurant, choose the tool, click constructed space in the tower, then confirm **Place office** or **Place restaurant** on the proposed room. Each room uses its predefined size. The first click spends nothing; Cancel, Escape, or a different tool discards the proposal. Pan and zoom preserve the pending world location. Floor span sizing and elevator service-range editing retain their existing controls, which open when the corresponding tool is selected.
