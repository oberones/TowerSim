...
export function mountGame(root:HTMLElement,options:MountOptions={}):()=>void {
  ...
  mounted.add(root);let view:GameView;let disposed=false;let diagnostics:{draw:()=>void;dispose:()=>void}|undefined;let diagnosticEpoch=0;const repository=options.repository??new IndexedDbSaveRepository();
  ...
  function remount():void {if(disposed)return;diagnostics?.dispose();diagnostics=undefined;view.dispose();view=compose();attachDiagnostics();}
  /** Load optional diagnostic code only in development, after each presentation replacement. */
  function attachDiagnostics():void {const epoch=++diagnosticEpoch;if(import.meta.env.DEV){void import('./ui/diagnostics').then(({mountDiagnostics})=>{if(!disposed&&epoch===diagnosticEpoch)diagnostics=mountDiagnostics(root,session);});}}
  try{view=compose();session.start(()=>{view.draw();diagnostics?.draw();});attachDiagnostics();}catch(error){session.dispose();mounted.delete(root);throw error;}
  return ()=>{if(disposed)return;disposed=true;diagnostics?.dispose();diagnostics=undefined;session.dispose();view.dispose();mounted.delete(root);};
}
