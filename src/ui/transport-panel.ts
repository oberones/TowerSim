...
export function createTransportPanel(session:GameSession,canvas:HTMLCanvasElement,camera:Camera,renderer:Renderer,tools:Map<string,HTMLButtonElement>,redraw:()=>void,signal:AbortSignal){
 ...
 for(const id of ['floor','demolish','inspect','office.small'])tools.get(id)!.addEventListener('click',()=>{mode=null;stair.cancel();elevator.cancel();for(const id of ['stairs.basic','elevator.standard'])tools.get(id)!.setAttribute('aria-pressed','false');},{signal});
