...
export function createOfficePanel(session:GameSession,canvas:HTMLCanvasElement,camera:Camera,renderer:Renderer,tools:Map<string,HTMLButtonElement>,redraw:()=>void,signal:AbortSignal){
 ...
 for(const id of ['floor','demolish','inspect','stairs.basic','elevator.standard'])tools.get(id)!.addEventListener('click',()=>{enabled=false;tool.cancel();officeButton.setAttribute('aria-pressed','false');},{signal});
 ...
 function draw():void {const q=tool.preview();commit.disabled=!q?.ok;if(enabled&&q){quote.textContent=`${q.ok?'Valid office':q.message??q.code} · Build ${money(q.quote?.constructionCostMinor??0)}`;const f=q.quote?.footprint;if(f){const a=camera.worldToScreen({x:f.startX,y:f.floor}),b=camera.worldToScreen({x:f.endXExclusive,y:f.floor+1}),ctx=renderer.overlayContext();ctx.strokeStyle=q.ok?'#80efc0':'#ff997d';ctx.lineWidth=2;ctx.strokeRect(a.x,b.y,b.x-a.x,a.y-b.y);}}
