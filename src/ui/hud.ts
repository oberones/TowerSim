...
export function createHud(session:GameSession){
  ...
  const update=()=>{const h=session.hud(),workforce=session.workforce();summary.textContent=`Day ${h.day} · ${h.time} · Level ${h.level} · ${money(h.cashMinor)} · ${h.speed===0?'Paused':`${h.speed}× speed`} · Workers ${workforce.assigned} / present ${workforce.present} · ${h.unsaved?'Unsaved tower':'Saved'}`;seed.textContent=`Seed ${h.seed}`;summary.classList.toggle('warning',h.cashMinor<0);};
