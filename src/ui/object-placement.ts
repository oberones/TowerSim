import {createFloorBoundaries} from './floor-boundaries';
...
export interface PlacementTool {propose:(floor:number,x:number,height:number,width:number)=>CommandResult;preview:()=>CommandResult|null;commit:()=>CommandResult;cancel:()=>void}
/** Preview, reposition and confirm any room or connection through one consistent on-object control. */
export function createObjectPlacement(canvas:HTMLCanvasElement,camera:Camera,renderer:Renderer,tools:Map<string,HTMLButtonElement>,definitionId:string,label:string,width:number,tool:PlacementTool,redraw:()=>void,signal:AbortSignal,defaultHeight=1,resizable=false,resizableWidth=false){
 ...
 node.append(quote,confirm,cancel,move,element('p',resizableWidth?'Drag the floor to move it. Grab its left or right edge to resize.':'Drag the preview or Move to position. Arrow keys on Move adjust one cell.','muted'));canvas.parentElement!.append(node);
 ...
 /** Share the current logical footprint between whole-object dragging and independent edge controls. */
 const bounds=()=>enabled&&anchor?{...anchor,width,height}:null;
 const boundaries=resizableWidth?createFloorBoundaries(canvas,camera,bounds,(x,nextWidth)=>{if(!anchor)return;width=nextWidth;reposition(anchor.floor,x);},signal):null;
 bindPlacementDrag(canvas,move,camera,bounds,reposition,signal);
 /** Requote the moved footprint without committing or changing its size. */
 function reposition(floor:number,x:number):void {if(!enabled)return;anchor={floor,x};if(resizable)top.value=String(floor+height-1);tool.propose(floor,x,height,width);redraw();}
 ...
 function draw():void {
  ...
  const visible=b.x>=0&&a.x<=camera.width&&a.y>=0&&b.y<=camera.height;node.hidden=!visible;boundaries?.draw(visible);
  node.style.left=`${Math.max(8,Math.min(camera.width-node.offsetWidth-8,resizableWidth?(a.x+b.x-node.offsetWidth)/2:b.x+node.offsetWidth+16<camera.width?b.x+8:a.x-node.offsetWidth-8))}px`;
  node.style.top=`${Math.max(8,Math.min(camera.height-node.offsetHeight-8,resizableWidth?b.y-node.offsetHeight-12:a.y-camera.floorPixels*camera.zoom/2-node.offsetHeight/2))}px`;
 }
 /** Abandon the presentation proposal without issuing a command or retaining a stale confirmation. */
 function reset():void {enabled=false;anchor=null;height=defaultHeight;width=defaultWidth;tool.cancel();boundaries?.draw(false);node.hidden=true;b.setAttribute('aria-pressed','false');}
 return {draw,reset};
}
