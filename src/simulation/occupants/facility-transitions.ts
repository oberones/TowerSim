import { admitRestaurant } from '../facilities/restaurant-admission';
import { customerVisit } from '../facilities/restaurants';
import { setWorkStatus } from './worker-lifecycle';
...
export function pursueGoal(state:GameState,p:Occupant):void {
 ...
 const office=p.goal.kind==='office'?state.offices[p.goal.facilityId]:p.goal.kind==='restaurant'?state.restaurants[p.goal.facilityId]:null,lobby=state.tower!.lobby;
 ...
 if(p.goal.kind==='restaurant'){if(!admitRestaurant(state,p))departWorker(state,p);return;}
 if(p.tripId){finishTrip(state,state.trips[p.tripId]!,'completed',state.clock.tick);p.tripId=null;}
 if(office){transition(p,{state:'insideFacility',location:{kind:'facility',facilityId:office.id}});if(p.schedule)setWorkStatus(state,p,'admitted');}
 else{const visit=p.kind==='customer'?customerVisit(state,p.id):null;if(visit)visit.status='departed';transition(p,{state:'outside',location:{kind:'outside'}});p.goal={kind:'none'};if(p.schedule&&p.schedule.status!=='canceled')setWorkStatus(state,p,'departed');retireOccupant(state,p);}
}
...
export function arriveWorker(state:GameState,p:Occupant):void {
 ...
 if(!o||!accessAt(state,o.floor,o.entranceX2).accessible||state.clock.tick>=p.schedule.departureTick){setWorkStatus(state,p,'skipped');return;}
 const lobby=state.tower!.lobby;p.goal={kind:'office',facilityId:o.id};p.tripId=startTrip(state,p.id,'officeArrival').id;setWorkStatus(state,p,'traveling');
...
export function departWorker(state:GameState,p:Occupant):void {
 ...
 const abandoned=p.state!=='insideFacility';if(p.tripId)finishTrip(state,state.trips[p.tripId]!,'abandoned',state.clock.tick);
 p.tripId=startTrip(state,p.id,abandoned?'abandonedExit':p.kind==='customer'?'restaurantExit':'officeExit').id;p.goal={kind:'exit'};p.journey=null;
 ...
 if(p.location.kind==='facility'){const o=(state.offices[p.location.facilityId]??state.restaurants[p.location.facilityId])!;transition(p,{state:'entering',location:{kind:'anchor',at:{floor:o.floor,x2:o.entranceX2}}});}
