...
export function pursueGoal(state:GameState,p:Occupant):void {
 ...
 else{transition(p,{state:'outside',location:{kind:'outside'}});p.goal={kind:'none'};if(p.schedule&&p.schedule.status!=='canceled')setWorkStatus(state,p,'departed');retireOccupant(state,p);}
}
