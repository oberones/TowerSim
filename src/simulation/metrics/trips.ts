import { recordFinishedTrip } from './daily-report';
...
export function finishTrip(state:GameState,trip:Trip,outcome:'completed'|'abandoned',atTick:number):void {if(trip.endTick!==null)return;closeSegment(trip,atTick);trip.endTick=atTick;trip.outcome=outcome;recordFinishedTrip(state,trip);}
...
export function pruneTrips(state:GameState):void {const retained=new Set(state.workforceDays.flatMap(d=>d.members.flatMap(m=>m.tripId?[m.tripId]:[])));const cutoff=state.clock.tick-(state.scenario.content?.historyLimits.days??30)*state.scenario.dayTicks;for(const trip of Object.values(state.trips))if(trip.endTick!==null&&trip.endTick<cutoff&&!retained.has(trip.id))delete state.trips[trip.id];}
