...
/** Quote a complete known office footprint without allocating IDs, schedules or lease demand. */
export function quoteFacility(state:GameState,p:FacilityPayload):CommandResult {
 ...
 if(p.definitionId!=='office.small')return {ok:false,code:'notImplemented',message:'Only office rooms are available in this phase.'};
 const d=officeDefinition(state),w=state.scenario.world!;
 ...
 if(!Number.isSafeInteger(p.floor)||!Number.isSafeInteger(p.x)||p.floor<w.minFloor||p.floor>w.maxFloor||p.x<0||quote.footprint.endXExclusive>w.widthCells)return reject('outOfBounds','Office footprint must fit within the world.');
 if(!covers(state.tower.floors.find(f=>f.level===p.floor)?.constructedRanges??[],quote.footprint))return reject('missingFloor','Construct the entire office floor first.');
 ...
 if(d.constructionCostMinor>0&&state.economy.balanceMinor<d.constructionCostMinor)return reject('insufficientFunds','Not enough cash for this office.');
...
export function quoteOfficeRemoval(state:GameState,id:string):CommandResult {
 const o=state.offices[id];if(!o)return {ok:false,code:id===state.tower?.lobby.id?'protectedBase':'invalidCommand',message:'Select an existing removable office.'};
 const a=accruedAmounts(state,o),delta=a.incomeMinor-a.costMinor;
 try{const draft=clonePlain(state);removeOffice(draft,id);topologyChanged(draft);}catch{return {ok:false,code:'overflow',message:'The accrued settlement or exit cannot fit within safe limits.'};}
...
export function commitFacility(state:GameState,p:FacilityPayload|string,sequence:number):CommandResult {
 ...
 if(typeof p==='string')removeOffice(draft,p);else{const d=officeDefinition(draft),id=allocateId('facility',draft.ids.entity);draft.offices[id]={id,typeId:'office.small',definitionVersion:1,floor:p.floor,x:p.x,width:d.footprint.width,height:1,entranceX2:p.x*2+d.entranceOffsetX2,createdTick:draft.clock.tick,generation:0,lease:null,accrual:createAccrual(draft.clock.tick)};if(d.constructionCostMinor){const charge=post(draft,{source:`construction:command:${sequence}:office:${id}`,amountMinor:-d.constructionCostMinor});if(!charge.ok)throw Error('Charge failed');}}
