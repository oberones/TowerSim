...
export function commitStair(state:GameState,p:StairPayload|string,sequence:number):CommandResult {
 ...
 try{const draft=clonePlain(state);if(typeof p==='string')delete draft.stairs[p];else{const d=draft.scenario.content!.definitions.find(d=>d.typeId===p.definitionId)!,id=allocateId('stair',draft.ids.entity),x2=p.x*2+d.entranceOffsetX2;
