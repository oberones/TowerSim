import { operatingPosting } from './history';
import type { FinanceArchive,OperatingDay } from './history';
...
export interface EconomyState {initialMinor:number;balanceMinor:number;transactions:Transaction[];archive:FinanceArchive;operatingDays:OperatingDay[]}
/** Preflight exact balance and allocation, then post one idempotent source transaction atomically. */
export function post(state:GameState,input:{source:string;amountMinor:number;operatingDay?:number}):{ok:true;id:string;replayed:boolean}|{ok:false;code:'invalidState'|'invalidNumber'|'overflow'|'transactionConflict'} {
  try {
    ...
    const days=operatingPosting(state,transaction,input.operatingDay??Math.floor(state.clock.tick/state.scenario.dayTicks));
    state.economy.operatingDays=days;state.economy.transactions.push(transaction);state.economy.balanceMinor=balance;state.ids.transaction.next=counter.next;
...
export function reconcile(economy:EconomyState):boolean {
  return BigInt(economy.initialMinor)+BigInt(economy.archive.netMinor)+economy.transactions.reduce((sum,t)=>sum+BigInt(t.amountMinor),0n)===BigInt(economy.balanceMinor);
}
