...
export function settleShaft(state:GameState,s:Shaft):void {const cost=shaftCost(state,s);if(cost&&!post(state,{source:`elevator:${s.id}:${s.costGeneration}:cost`,amountMinor:-cost,operatingDay:Math.floor((state.clock.tick-1)/state.scenario.dayTicks)}).ok)throw Error('Elevator operating settlement failed');s.costSinceTick=state.clock.tick;s.costGeneration=add(s.costGeneration,1);}
