...
import { stopQueue,findEntry,appendEntry,removeEntry } from './queue';
...
export function requestElevator(state:GameState,occupantId:string,tripId:string,boardingStopId:string,unloadStopId:string):RequestResult {
 ...
 if(p.location.kind==='car'||queueIndex(state).person(occupantId)||Object.values(state.cars).some(c=>c.onboard.some(o=>o.occupantId===occupantId)))return {ok:false,code:'conflictingMembership'};
 ...
 const queue=stopQueue(state,boardingStopId,to.floor>from.floor?'up':'down')!,next=add(state.ids.queueAdmission.next,1),id=allocateId('entry',state.ids.entity);appendEntry(state,queue,{id,occupantId,tripId,unloadStopId,joinedTick:state.clock.tick,admissionSequence:state.ids.queueAdmission.next,reservedVisitId:null,lastDeniedVisitId:null});state.ids.queueAdmission.next=next;
...
export function cancelElevatorRequest(state:GameState,entryId:string,_reason:'goalChanged'|'serviceRemoved'):boolean {
 ...
 closeSegment(state.trips[entry.tripId]!,state.clock.tick);removeEntry(state,entryId);transition(p,{state:'entering',location:{kind:'anchor',at:{floor:stop.floor,x2:stop.anchorX2}}});if(p.journey)p.journey.legs=[];p.replanAfterCurrentLeg=true;return true;
}
