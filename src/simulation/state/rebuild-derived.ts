...
export function rebuildDerived(state:GameState):RuntimeContext {assertState(state);return {...activeIndex(state),graph:buildGraph(state),routes:new RouteCache(state.scenario.content?.routing.routeCacheEntries??1),walking:buildWalkingSpace(state),scheduler:new Scheduler(state.scheduledEvents,state.clock.tick)};}
