...
export class Renderer {
  ...
  draw(view:WorldView):void {
    ...
    if(key!==this.key){this.key=key;for(const canvas of [this.canvas,this.cache]){canvas.width=Math.round(c.width*c.dpr);canvas.height=Math.round(c.height*c.dpr);}this.cached.setTransform(c.dpr,0,0,c.dpr,0,0);drawStructure(this.cached,view,c);this.staticDraws++;}
    this.context.setTransform(1,0,0,1,0,0);this.context.clearRect(0,0,this.canvas.width,this.canvas.height);this.context.drawImage(this.cache,0,0);this.context.setTransform(c.dpr,0,0,c.dpr,0,0);
  }
